...
fn rewrite_pattern_types(
    ...
    match &mut pattern.kind {
        ...
        PatternKind::Some { value } => rewrite_pattern_types(value, context)?,
...
fn collect_pattern_bindings(pattern: &Pattern, out: &mut Vec<String>) {
    match &pattern.kind {
        ...
        PatternKind::Some { value } => collect_pattern_bindings(value, out),
...
fn rename_top_pattern(pattern: &mut Pattern, values: &BTreeMap<String, Symbol>) {
    match &mut pattern.kind {
        ...
        PatternKind::Some { value } => rename_top_pattern(value, values),
        PatternKind::Or { patterns } => {

...
impl<'a> FunctionLowerer<'a> {
        ...
        match kind {
            ...
            TypedExprKind::BuiltinConstructor { constructor, .. } => {
                let HirExprKind::Call { args, .. } = &expr.kind else {
                    self.diagnostic(expr.span, "fir/constructor-shape", "resolved constructor is not a call");
                    return self.poison(expr.span, result_ty);
                };
                let value = if let Some(value) = first_positional(args) {
                    self.lower_expr(value)
                } else if args.is_empty() && match (constructor, &result_ty) {
                    (ResolvedBuiltinValue::Ok, Ty::Result { ok, .. }) => **ok == Ty::Void,
                    (ResolvedBuiltinValue::Err, Ty::Result { error, .. }) => **error == Ty::Void,
                    _ => false,
                } {
                    self.emit_value(expr.span, Ty::Void, FirInstructionKind::Unit)
                } else {
                    self.diagnostic(expr.span, "fir/constructor-arguments", "resolved constructor lacks a positional payload");
                    return self.poison(expr.span, result_ty);
                };
                match constructor {
                    ResolvedBuiltinValue::Some => self.emit_value(
                        expr.span, result_ty, FirInstructionKind::MakeSome { value },
                    ),
                    ResolvedBuiltinValue::Ok => self.emit_value(
                        expr.span, result_ty, FirInstructionKind::MakeResultOk { value },
                    ),
                    ResolvedBuiltinValue::Err => self.emit_value(
                        expr.span, result_ty, FirInstructionKind::MakeResultErr { error: value },
                    ),
                }
            }
    ...
    fn lower_match_test(
        ...
        match test {
            ...
            MatchTest::ResultOk => self.emit_value(span, Ty::Bool, FirInstructionKind::ResultIsOk { value }),
            MatchTest::ResultErr => {
                let ok = self.emit_value(span, Ty::Bool, FirInstructionKind::ResultIsOk { value });
                self.emit_value(span, Ty::Bool, FirInstructionKind::Unary { op: FirUnaryOp::Not, value: ok })
            }
    ...
    fn lower_match_projection(
        ...
        for projection in projections {
            value = match projection {
                ...
                MatchProjection::ResultPayload { ty, ok } => {
                    self.emit_value(
                        span,
                        ty.clone(),
                        if *ok {
...
fn first_bound_local(pattern: &HirPattern) -> Option<LocalId> {
    match &pattern.kind {
        ...
        HirPatternKind::Some { value } | HirPatternKind::Ok { value } | HirPatternKind::Err { value } => first_bound_local(value),
        HirPatternKind::Sequence { items, rest } => {

use forge_frontend::{
    lower_module, lower_resolved_bodies, parse_source, ResolvedBuiltinValue, ResolvedName,
};
...
fn resolves_cosmic_sia32_privileged_source_names() {
    ...
    fn collect(expr: &forge_frontend::body_hir::HirExpr, out: &mut Vec<ResolvedBuiltinValue>) {
        ...
        if let HirExprKind::Call { callee, args } = &expr.kind {
            if let HirExprKind::Name { reference } = &callee.kind {
                if let ResolvedName::BuiltinValue(value) = reference.root {
                    out.push(value);
                }
            }
            for arg in args {
                collect(
                    match arg {
                        forge_frontend::body_hir::HirCallArg::Positional { value } => value,
                        forge_frontend::body_hir::HirCallArg::Named { value, .. } => value,
                    },
                    out,
                );
            }
    ...
    for stmt in &body.block.statements {
        if let forge_frontend::body_hir::HirStmtKind::Expr { expr } = &stmt.kind {
            collect(expr, &mut builtins);
        }
    }

use forge_frontend::{
    lower_module, parse_source, resolve_module_bodies, DefId, ResolvedName,
};
...
fn resolves_parameter_and_shadowed_local() {
    ...
    let x_uses = body.uses.iter().filter(|u| u.name == "x").collect::<Vec<_>>();
...
fn rejects_duplicate_local_in_same_scope() {
    ...
    "#);
    assert!(output.diagnostics.iter().any(|d| d.message.contains("duplicate local definition `x`")));
}
...
fn resolves_forward_top_level_function() {
    ...
    assert!(output.diagnostics.is_empty(), "{:?}", output.diagnostics);
    let body = &output.bodies[&DefId(0)];
    let second = body.uses.iter().find(|u| u.name == "second").expect("second use");
...
fn reports_unresolved_value_and_type_names() {
    ...
    "#);
    assert!(output.diagnostics.iter().any(|d| d.message.contains("unresolved type name `MissingType`")));
    assert!(output.diagnostics.iter().any(|d| d.message.contains("unresolved value name `missing_value`")));
}
...
fn pattern_bindings_enter_arm_scope() {
    ...
    assert!(body.uses.iter().any(|u| u.name == "x" && matches!(u.resolution, ResolvedName::Local(_))));
}
...
fn builtin_types_resolve_without_module_symbols() {
    ...
    assert!(output.diagnostics.is_empty(), "{:?}", output.diagnostics);
    let body = &output.bodies[&DefId(0)];
    assert!(body.uses.iter().any(|u| u.name == "u32" && u.resolution == ResolvedName::BuiltinType));
    assert!(body.uses.iter().any(|u| u.name == "i64" && u.resolution == ResolvedName::BuiltinType));
}

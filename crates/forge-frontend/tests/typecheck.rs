...
#[test]
fn rejects_bitwise_not_on_non_integer_before_fir_lowering() {
    let output = check(
        r#"
        module test.bitwise_not_bool;
        fn main() -> i32 {
            val bad = ~true;
            return 0;
        }
    ...
    let diagnostic = output
        ...
        .iter()
        .find(|diagnostic| diagnostic.code == "type/mismatch")
        .expect("bitwise-not operand diagnostic");
    assert!(
        diagnostic.message.contains("invalid unary operand Bool"),
        "{:?}",
        output.diagnostics
    );
}
...
#[test]
fn rejects_boolean_literal_with_integer_declared_type() {
    let output = check(
        r#"
        module test.boolean_literal_integer_type;
        fn main() -> i32 {
            val bad: u32 = true;
            return 0;
        }
...
#[test]
fn rejects_duration_reader_with_integer_declared_type() {
    let output = check(
        r#"
        module test.duration_reader_integer_type;
        fn main() -> i32 {
            val bad: u32 = #duration "1ms";
            return 0;
        }
    "#,
    );

...
use cranelift_codegen::ir::{
    AbiParam, Block, Function, InstBuilder, MemFlags, MemFlagsData, Signature, StackSlot,
...
fn lower_pointer_offset(
    ...
    let normalized_offset = normalize_integer_to_type(offset_ty, raw_offset, pointer_clif, types, cursor)?;
...
fn lower_integer_convert(
    ...
    normalize_integer_to_type(source, lookup_value(values, input)?, types.value_type(target)?, types, cursor)
}

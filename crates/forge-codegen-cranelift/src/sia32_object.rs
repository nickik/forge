...
impl Sia32Section {
    ...
    fn from_tag(tag: u8) -> Result<Self, BackendError> {
        match tag {
            ...
            2 => Ok(Self::Rodata),
            3 => Ok(Self::Data),
            _ => Err(sia32_object_error(format!(
                "unknown SIA32 object section tag {tag}"
            ))),
        }
    }
}
...
impl Sia32Relocation {
    pub const fn section(&self) -> Sia32Section {
        self.section
    }
...
impl Sia32Object {
    ...
    pub fn section(&self, section: Sia32Section) -> &[u8] {
        match section {
            Sia32Section::Text => &self.text,
            Sia32Section::Rodata => &self.rodata,
    ...
    pub fn define_section_symbol(
        ...
        if u64::from(offset) > self.section(section).len() as u64 {
            return Err(sia32_object_error(format!(
                "SIA32 symbol {name:?} offset {offset} exceeds {section:?} size {}",
                self.section(section).len()
            )));
        }
        match self.symbols.entry(name) {
            Entry::Vacant(entry) => {
                entry.insert(Sia32Symbol { section, offset });
            }
    ...
    pub fn add_section_abs32_relocation(
        &mut self,
        section: Sia32Section,
        offset: u32,
        symbol: impl Into<String>,
    ...
    pub fn add_section_relocation(
        ...
        if u64::from(offset) + 4 > self.section(section).len() as u64 {
            return Err(sia32_object_error(format!(
                "SIA32 ABS32 relocation at {offset} exceeds {section:?} size {}",
                self.section(section).len()
            )));
        }
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, BackendError> {
        ...
        put_u32(
            &mut out,
            checked_u32(self.relocations.len(), "relocation count")?,
        );
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, BackendError> {
        ...
        if input.take(8)? != SIAO_MAGIC {
            return Err(sia32_object_error("invalid SIAO32 object magic/version"));
        }
        ...
        for _ in 0..relocation_count {
            let section = Sia32Section::from_tag(input.u8()?)?;
            if input.u8()? != 1 {
                return Err(sia32_object_error("unsupported SIAO32 relocation tag"));
            }
            let offset = input.u32()?;
        ...
        if !input.is_empty() {
            return Err(sia32_object_error("trailing bytes after SIAO32 object"));
        }
        Ok(object)
    }
}
...
pub fn link_sia32_sectioned_objects(
    objects: &[Sia32Object],
    bases: &[Sia32SectionBases],
) -> Result<Vec<LinkedSia32Object>, BackendError> {
    if objects.len() != bases.len() {
        return Err(sia32_object_error(format!(
            "SIA32 linker received {} objects but {} base layouts",
            objects.len(),
            bases.len()
        )));
    }
    ...
    for (object, base) in objects.iter().zip(bases) {
        for (name, symbol) in object.symbols() {
            ...
            let address = section_base.checked_add(symbol.offset).ok_or_else(|| {
    ...
    let mut linked = objects
        .iter()
        .map(|o| LinkedSia32Object {
            text: o.text.clone(),
            rodata: o.rodata.clone(),
            data: o.data.clone(),
        })
        .collect::<Vec<_>>();
    for (object, output) in objects.iter().zip(&mut linked) {
        for relocation in object.relocations() {
            ...
            let bytes = match relocation.section {
                Sia32Section::Text => &mut output.text,
                Sia32Section::Rodata => &mut output.rodata,
                Sia32Section::Data => &mut output.data,
            };
...
pub fn link_sia32_objects(
    ...
    let layouts = bases
        .iter()
        .copied()
        .map(|base| Sia32SectionBases {
            text: base,
            rodata: base,
            data: base,
        })
        .collect::<Vec<_>>();
    Ok(link_sia32_sectioned_objects(objects, &layouts)?
        .into_iter()
        .map(|o| o.text)
        .collect())
}

fn checked_u32(value: usize, what: &str) -> Result<u32, BackendError> {
    u32::try_from(value).map_err(|_| sia32_object_error(format!("SIAO32 {what} exceeds u32")))
}
fn put_u32(out: &mut Vec<u8>, value: u32) {
    out.extend_from_slice(&value.to_le_bytes());
}
fn put_string(out: &mut Vec<u8>, value: &str) -> Result<(), BackendError> {
...
impl<'a> Input<'a> {
    ...
    fn take(&mut self, count: usize) -> Result<&'a [u8], BackendError> {
        let end = self
            .pos
            .checked_add(count)
            .ok_or_else(|| sia32_object_error("SIAO32 parse offset overflow"))?;
        let result = self
            .bytes
            .get(self.pos..end)
            .ok_or_else(|| sia32_object_error("truncated SIAO32 object"))?;
        self.pos = end;
        Ok(result)
    }
    ...
    fn string(&mut self) -> Result<String, BackendError> {
        ...
        String::from_utf8(self.take(len)?.to_vec())
            .map_err(|_| sia32_object_error("non-UTF-8 SIAO32 symbol"))
    }

...
fn binary(op: BinaryOp, ty: Ty, overflow: Option<OverflowMode>) -> FirFunction {
    ...
    FirFunction {
        ...
        locals: BTreeMap::from([
            (p0, FirLocal { id: p0, source: None, ty: ty.clone(), mutable: false, parameter: true, synthetic: false }),
            (p1, FirLocal { id: p1, source: None, ty: ty.clone(), mutable: false, parameter: true, synthetic: false }),
        ]),
        ...
        blocks: vec![FirBasicBlock {
            ...
            closure: None,
            instructions: vec![
                FirInstruction { span, result: Some(v0), kind: FirInstructionKind::Load { place: FirPlace::Local { local: p0 } } },
                FirInstruction { span, result: Some(v1), kind: FirInstructionKind::Load { place: FirPlace::Local { local: p1 } } },
                FirInstruction { span, result: Some(v2), kind: FirInstructionKind::Binary { op, overflow, left: v0, right: v1 } },
            ],
...
fn unary(op: FirUnaryOp, ty: Ty) -> FirFunction {
    ...
    FirFunction {
        ...
        locals: BTreeMap::from([(
            p0,
            FirLocal { id: p0, source: None, ty: ty.clone(), mutable: false, parameter: true, synthetic: false },
        )]),
        closures: BTreeMap::new(),
        entry: FirBlockId(0),
        blocks: vec![FirBasicBlock {
            ...
            instructions: vec![
                FirInstruction { span, result: Some(v0), kind: FirInstructionKind::Load { place: FirPlace::Local { local: p0 } } },
                FirInstruction { span, result: Some(v1), kind: FirInstructionKind::Unary { op, value: v0 } },
            ],
...
fn conversion(source: Ty, target: Ty) -> FirFunction {
    ...
    FirFunction {
        ...
        locals: BTreeMap::from([(
            p0,
            FirLocal { id: p0, source: None, ty: source.clone(), mutable: false, parameter: true, synthetic: false },
        )]),
        closures: BTreeMap::new(),
        entry: FirBlockId(0),
        blocks: vec![FirBasicBlock {
            ...
            instructions: vec![
                FirInstruction { span, result: Some(v0), kind: FirInstructionKind::Load { place: FirPlace::Local { local: p0 } } },
                FirInstruction { span, result: Some(v1), kind: FirInstructionKind::Convert { value: v0, target: target.clone() } },
            ],
            terminator: Some(FirTerminator::Return { value: Some(v1) }),
        }],
...
fn div_and_rem_select_signedness() {
    for backend in [
        ...
        for (signed, div, rem) in [(true, "sdiv", "srem"), (false, "udiv", "urem")] {
            ...
            let div_clif = lower(binary(BinaryOp::Div, ty.clone(), Some(OverflowMode::Checked)), rebuild(&backend));
            let rem_clif = lower(binary(BinaryOp::Rem, ty.clone(), Some(OverflowMode::Checked)), rebuild(&backend));
...
fn bitwise_ops_lower_directly() {
    ...
    for (op, expected) in [
        ...
        let clif = lower(binary(op, ty.clone(), None), CraneliftBackend::aarch64().expect("AArch64"));
...
fn shifts_check_count_and_select_signed_right_shift() {
    ...
    let left = lower(
        binary(BinaryOp::ShiftLeft, signed.clone(), Some(OverflowMode::Checked)),
        CraneliftBackend::aarch64().expect("AArch64"),
    );
    ...
    assert!(signed_right.contains("sshr") && signed_right.contains("trapnz"), "{signed_right}");
    assert!(unsigned_right.contains("ushr") && unsigned_right.contains("trapnz"), "{unsigned_right}");
}
...
fn unary_negation_and_bitnot_lower() {
    ...
    let neg = lower(unary(FirUnaryOp::Neg, ty.clone()), CraneliftBackend::aarch64().expect("AArch64"));
    let not = lower(unary(FirUnaryOp::BitNot, ty), CraneliftBackend::aarch64().expect("AArch64"));
...
fn integer_conversions_use_reduce_sign_extend_and_zero_extend() {
    ...
    let sext = lower(conversion(signed8, signed64.clone()), CraneliftBackend::aarch64().expect("AArch64"));
    let uext = lower(conversion(unsigned8, unsigned64.clone()), CraneliftBackend::aarch64().expect("AArch64"));
    let reduce = lower(conversion(unsigned64, int_ty(false, IntWidth::W16)), CraneliftBackend::aarch64().expect("AArch64"));
...
fn rebuild(backend: &CraneliftBackend) -> CraneliftBackend {
    match backend.target() {
        forge_codegen_cranelift::CraneliftTarget::Aarch64 => CraneliftBackend::aarch64().expect("AArch64"),
        forge_codegen_cranelift::CraneliftTarget::Riscv64 => CraneliftBackend::riscv64().expect("RV64"),
    }
}
